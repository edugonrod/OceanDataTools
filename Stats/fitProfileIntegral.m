...
if nargin<4||isempty(npts),npts=200;end
...
[xmax,im]=max(xi);
zmax=zi(im);
x_model=interp1(zi,xi,z,'linear');
res=x-x_model;
...
profile.r2=1-sum(res.^2)/sum((x-mean(x)).^2);
